...
export const CardParallax = {
  ...
  updateCardParallax(e) {
    const { x: mouseX, y: mouseY } = utils.calculateMousePosition(
      e,
    ...
    elements.background.style.transform = `translate(${
      mousePX * backgroundMoveFactor
    }px, ${mousePY * backgroundMoveFactor}px)`;

    const contentTransform = `translate(${mousePX * contentMoveFactor}px, ${
      mousePY * contentMoveFactor
    }px)`;
    elements.content.style.transform = contentTransform;
    elements.spotlight.style.transform = contentTransform;

    elements.profile.style.transform = `translate(${
      mousePX * profileMoveFactor
    }px, ${mousePY * profileMoveFactor}px)`;
  },
  ...
  updateContentSpotlight(e) {
    ...
    if (utils.isPointInRect(mousePos, contentRect)) {
      ...
      const spotlightWidth =
        contentRect.width * config.parallax.spotlightSizeIncrease;
      const spotlightHeight =
        contentRect.height * config.parallax.spotlightSizeIncrease;
